...
export class BlogService {
  ...
  getAll(blogPaging: BlogPaging) : Observable<PagedResult<Blog>> {
    return this.http.get<PagedResult<Blog>>(
      environment.webApi + '/Blog?Page=' + blogPaging.page.toString() + '&PageSize=' + blogPaging.pageSize.toString());
  }

  get(blogId: number) : Observable<Blog> {
    return this.http.get<Blog>(environment.webApi + '/Blog/' + blogId.toString());
  }

  getByApplicationUserId(applicationUserId : number) : Observable<Blog[]> {
    return this.http.get<Blog[]>(environment.webApi + '/Blog/user/' + applicationUserId.toString());
  }
  ...
  delete(blogId : number) : Observable<number> {
    return this.http.delete<number>(environment.webApi + '/Blog/' + blogId.toString());
  }
}
