...
namespace BlogLab.Web.Controllers
{
    ...
    public class PhotoController : ControllerBase
    {
        ...
        {
            int applicationUserId = int.Parse(User.Claims.First(i => i.Type == JwtRegisteredClaimNames.NameId).Value);
            var photos = await _photoRepository.GetAllByUseridlAsync(applicationUserId);
            return Ok(photos);
        }

        //http://localhost:5000/api/photo/12
        [HttpGet("{photoId}")]
        ...
        public async Task<ActionResult<int>> Delete(int photoId)
        {
            ...
            if (foundPhoto != null)
            {
                if (foundPhoto.ApplicationUserid == applicationUserId)
                {
                    ...
                    if (usedInBlog) return BadRequest("Cannot remove photo being used in a blog");
